...
const errorMap = {
  ...
  '012': {
    code: '012',
    message: 'If the Default Payload includes exp, nbf or iat values - these must be set to an integer.',
    error: null
  }
}
 ...
 * @param  {object} defaultPayload default payload properties and directives for registered claims
 * @return {String}           jwt
 */
jwt.encode = function encode (payload, secret, algorithm = 'HS256', header = { type: 'JWT' }, defaultPayload = {} ) {
  // check secret
  if (!secret) {
  ...
  // defaultPayload
  if ( defaultPayload ) {
    for( var key in defaultPayload ) {
      if ( !defaultPayload.hasOwnProperty(key) ) { // IGNORE PROTOTYPE PROPERTIES
        continue
      }
      if ( typeof payload[key]!=='undefined' ) { // NEVER OVERWRITE EXISTING PAYLOAD PROPERTIES
        continue
      }
      if ( key=='exp' || key=='iat' || key=='nbf' ) { // HANDEL UNIX TIME RESERVED CALIMS
        var offset = parseInt(defaultPayload[key],10)
        if ( isNaN(offset) || (defaultPayload[key]+'').indexOf('.')!=-1 ) {
          throw new Error(errorMap['012'].message)
        }
        payload[key] = Date.now()/1000 + (key=='nbf'?-1:1) * offset
        continue
      }
      if ( key=='jti' ) {
        if ( defaultPayload[key]===false ) { // DON'T INCLUDE jti IF FALSE IS PASSED
          continue
        }
        if ( defaultPayload[key]===true ) { // A DEFAULT ID FUNCTION (UUID LIB SHOULD BE USED IF NOT ADEQUATELY RANDOM)
          payload[key] = crypto.randomBytes(32).toString('base64')
          continue
        }
        if ( typeof defaultPayload[key] === 'function' ) { // EXECUTE USERS ID GENERATING FUNCTION
...
/**
 * hmac sign
 * @param  {String} input     hmac sign input
 * @param  {String} algorithm hmac sign algorithm
 * @param  {String} secret    hmac sign secret
 ...
 * @param  {String} input      hmac verify input
 * @param  {String} algorithm  hmac verify algorithm
 * @param  {String} secret     hmac verify secret
 * @param  {String} signature  hmac verify signature
 * @return {Object}            hmac verify result
 */
function hmacVerify (input, algorithm, secret, signature) {
  try {
    return signature === hmacSign(input, algorithm, secret)
  } catch (e) {
    errorMap['006'].message = 'Exception error in hmac verify, signature verification failed'
