...
const errorMap = {
  ...
  '007': {
    code: '007',
    message: 'Token expired, signature verification failed',
    error: null
  },
  '008': {
    code: '008',
    message: 'The exp is unvalid, you must give a valid param for Date.parse()',
    error: null
  }
}
...
jwt.decode = function decode (token, secret, noVerify = false) {
  ...
  if (!noVerify) {
    ...
    // Support for exp claims, and so on
    if (payload.exp) {
      let exp = Date.parse(payload.exp)
